...
功能：查询账户的资金状况和持仓列表
用法：python get_portfolio.py --market HK --trd-env REAL
...
- show_option_strategy_view: True 时按期权策略维度展示持仓（默认 False）

返回字段说明：
- power（购买力）: 按 50% 融资初始保证金率计算的近似值
        ...
        if output_json:
            ...
            if positions:
                if show_option_strategy_view:
                    print(
                        f"  {'代码':<12} {'名称':<10} {'数量':>8} {'均价':>10} {'市值':>12} {'盈亏%':>8} "
                        f"{'策略类型':<12} {'持仓类型':<10}"
                    )
                    print("  " + "-" * 90)
                else:
