...
功能：查询当前账户的今日订单
用法：python get_orders.py --market HK --trd-env REAL
...
def get_orders(acc_id=None, market=None, trd_env=None, security_firm=None,
    ...
    try:
        ctx = create_sec_or_future_trade_context(
            trd_market, security_firm=parse_security_firm(security_firm), ctx_type=ctx_type
        )
...
if __name__ == "__main__":
    ...
    get_orders(acc_id=args.acc_id, market=args.market, trd_env=args.trd_env,
               security_firm=args.security_firm, ctx_type=args.ctx_type,
